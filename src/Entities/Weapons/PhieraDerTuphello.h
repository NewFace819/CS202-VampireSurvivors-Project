...
class PhieraDerTuphello : public WeaponBase {
public:
    // Base stats: Damage = 5, Cooldown = 1.4s, Speed = 100%, Amount = 1, Pierce = 1
    PhieraDerTuphello(float cooldown = 1.4f, float damage = 5.0f, float speed = 400.0f)
    ...
    std::string getName() const override { return "Phiera Der Tuphello"; }
...
protected:
    void fire(const sf::Vector2f& startPos, const sf::Vector2f& playerDir, const std::vector<EnemyBase*>& enemies, std::vector<Projectile>& activeProjectiles, int shotIndex = 0) override {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (m_hasVfxTex) {
                // BulletRed from vfx_atlas: x=1226, y=559, w=21, h=21 -> SFML y = 2048 - 559 - 21 = 1468
                sf::IntRect iconRect(1226, 1468, 21, 21);
                ...
                p.setCustomShape(sf::Vector2f(12.f, 12.f), sf::Color::Red);
            }
