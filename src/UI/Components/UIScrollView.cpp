...
void UIScrollView::Draw(sf::RenderTarget& target)
{
    ...
    // Use the actual render target size so viewport normalisation is always 1:1
    // regardless of which virtual-resolution constant the project defines.
    float targetW = static_cast<float>(target.getSize().x);
    float targetH = static_cast<float>(target.getSize().y);

    float nx = m_position.x / targetW;
    float ny = m_position.y / targetH;
    float nw = m_viewWidth  / targetW;
    float nh = m_viewHeight / targetH;
