...
MainBoardPanel::MainBoardPanel(TextureAtlas& atlas, const sf::Font& font, const sf::Font* boldFont, TextureAtlas* itemsAtlas)
    : UIPanel(atlas, "frame5_c4", 12, 12, 12, 12)
    , m_scrollView(nullptr)
    , m_rosterGridRaw(nullptr)
    , m_detailPanel(std::make_unique<DetailPanel>(atlas, font, boldFont, itemsAtlas))
{
    auto rosterGrid = std::make_unique<RosterGridPanel>(atlas, font, boldFont, itemsAtlas);
