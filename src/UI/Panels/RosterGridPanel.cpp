...
RosterGridPanel::RosterGridPanel(TextureAtlas& atlas, const sf::Font& font, const sf::Font* boldFont, TextureAtlas* itemsAtlas)
    : m_atlas(atlas)
    , m_font(font)
    , m_boldFont(boldFont)
...
void RosterGridPanel::InitializeRoster(const CharacterDataManager& dataManager, const PlayerProgressionManager& progManager, const WeaponDataManager* weaponManager, TextureAtlas* itemsAtlas)
{
    m_cards.clear();
    if (itemsAtlas) m_itemsAtlas = itemsAtlas; // override if provided
    ...
    for(const auto& id : order)
    {
        ...
        auto card = std::make_unique<CharacterCardWidget>(m_atlas, m_font, m_boldFont, profile, isUnlocked, weaponManager, m_itemsAtlas);
