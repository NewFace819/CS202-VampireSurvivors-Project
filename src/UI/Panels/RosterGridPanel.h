...
public:
    RosterGridPanel(TextureAtlas& atlas, const sf::Font& font, const sf::Font* boldFont = nullptr, TextureAtlas* itemsAtlas = nullptr);
    ~RosterGridPanel() override = default;

    void InitializeRoster(const CharacterDataManager& dataManager, const PlayerProgressionManager& progManager, const WeaponDataManager* weaponManager = nullptr, TextureAtlas* itemsAtlas = nullptr);
    void SetOnSelectionChanged(std::function<void(const std::string&)> callback);
    void SetCardConfirmedState(bool confirmed);
    void ClearSelection();
    void UnlockCard(const std::string& characterId);
